...
function parseOutput(text: string): StoryCandidate[] {
  ...
  const parsed = JSON.parse(trimmed) as { candidates?: StoryCandidate[] };
      ...
      input: [
        {
          role: "developer",
          content:
            "あなたは大学生向けサービス「CHATDIARY」の物語編集AIです。LINE風トーク履歴から、小さな出来事、空気感、関係性、感情の変化、物語になりそうな話題のまとまりを読み取り、短編小説シール候補を3件作ります。実名、学校名、住所、電話番号、URL、支払い情報は出さないでください。LINEの文を大量に引用せず、単なる要約ではなく、やさしく余韻のある短編小説として再構成してください。本文は各候補200〜350字程度。大学生が読んで自然な日本語にしてください。候補ごとに内容を変えてください。必ず指定JSONだけを返してください。",
        },
        {
          role: "user",
          content: `LINEトーク履歴:\n${talkText}`,
